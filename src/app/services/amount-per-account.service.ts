...
import {
    ...
    RxAmountsPerAccountDatabase,
    RxAmountsPerAccountCollections,
    RxAmountPerAccountDocumentType
} from '../RxDB';

import amountPerAccountSchema from '../schemas/amountPerAccount.schema';
 ...
 */
async function _create(): Promise<RxAmountsPerAccountDatabase> {

    await loadRxDBPlugins();
    console.log('DatabaseService: creating database..');
    //await removeRxDatabase('angularAmountPerAccount', 'idb');
    const db = await createRxDatabase<RxAmountsPerAccountCollections>({
        name: 'angularAmountPerAccount',
        adapter: 'idb'
    ...
    if (doSync) {
        ...
        await db.amountperaccount.sync({
            remote: syncURL + '/amountPerAccount'
        });
    }
...
let DB_INSTANCE: RxAmountsPerAccountDatabase;

/**
 * This is run via APP_INITIALIZER in app.module.ts
 * to ensure the database exists before the angular-app starts up
 */
export async function initAmountPerAccountDatabase() {
...
@Injectable()
export class AmountPerAccountService {
    get db(): RxAmountsPerAccountDatabase {
        return DB_INSTANCE;
    }
}
