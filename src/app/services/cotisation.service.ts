...
import {
    ...
    RxCotisationsDatabase,
    RxCotisationsCollections,
    RxCotisationDocumentType
} from '../RxDB';

import cotisationSchema from '../schemas/cotisation.schema';
...
let collections = [
    {
        ...
        methods: {
            hpPercent(this: RxCotisationDocument): string {
                return "test";
            }
        }, 
        migrationStrategies: {
 ...
 */
async function _create(): Promise<RxCotisationsDatabase> {

    await loadRxDBPlugins();

    console.log('DatabaseService: creating database..');
    //await removeRxDatabase('angularCotisation', 'idb');
    const db = await createRxDatabase<RxCotisationsCollections>({
        name: 'angularCotisation',
        adapter: 'idb'
    ...
    if (doSync) {
        ...
        await db.cotisation.sync({
            remote: syncURL + '/cotisation'
        });
    }
...
let DB_INSTANCE: RxCotisationsDatabase;

/**
 * This is run via APP_INITIALIZER in app.module.ts
 * to ensure the database exists before the angular-app starts up
 */
export async function initCotisationDatabase() {
